...
Sebelum memulai, pastikan perangkat Anda telah terpasang dependensi berikut:
...
1. **Clone repository ini:**
   ...
   ```sh
   git clone https://github.com/aldiipratama/digital-arsip-agy.git
   cd digital-arsip-agy
   ```

2. **Install dependensi Node.js (Frontend & Monorepo Tooling):**

   ```sh
   pnpm install
   ```

3. **Setup Backend Laravel (`apps/api`):**
...
4. **Konfigurasi Database & Jalankan Migrasi:**
   ...
   Sesuaikan variabel `DB_DATABASE`, `DB_USERNAME`, dan `DB_PASSWORD` di `apps/api/.env`, lalu jalankan:

   ```sh
   php artisan migrate --seed
   ```

5. **Setup Frontend Next.js (`apps/dashboard`):**

   ```sh
   cd ../dashboard
   cp .env.example .env.local  # atau buat .env.local
   ```
...
Anda dapat menjalankan Frontend (`apps/dashboard`) dan Backend (`apps/api`) secara bersamaan dari root monorepo:

```sh
# Dari root repository (digital-arsip-agy)
pnpm dev
```

Atau jalankan masing-masing layanan secara independen:
...
Perintah umum yang dapat digunakan dalam repositori monorepo ini:

```sh
# Jalankan development server
pnpm dev

# Build seluruh aplikasi (Next.js dashboard)
pnpm build

# Pengecekan tipe TypeScript di seluruh monorepo
pnpm check-types

# Format ulang kode menggunakan Prettier
pnpm format
```
