...
1. POST /api/auth/login (wrong password)
2. Login as Manager
3. GET /api/audit-logs?action=failed_login_attempt
```
...
2. GET /api/audit-logs?action=successful_login
```
...
3. Login as Manager
4. GET /api/audit-logs?action=account_locked
```
...
- [ ] Test frontend integration with CSRF
