...
const METRICS = [
  ...
  { key: "verified_documents", label: "Terverifikasi", color: "text-[#00A63E]" },
  { key: "pending_documents", label: "Menunggu", color: "text-[#D08700]" },
  { key: "rejected_documents", label: "Ditolak", color: "text-[#E7000B]" },
] as const;

export function DocumentListMetrics({ stats, isLoading }: DocumentListMetricsProps) {
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {METRICS.map((m) => (
          <Card key={m.key} className="border border-border/60 bg-card shadow-sm p-5 space-y-2">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {METRICS.map((m) => (
        <Card key={m.key} className="border border-border/60 bg-card shadow-sm p-5">
