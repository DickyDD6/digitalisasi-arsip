...
interface QCVerifiedDocumentItemProps {
  doc: DocumentItem;
  onOpenView?: (doc: DocumentItem) => void;
}

export function QCVerifiedDocumentItem({
  doc,
  onOpenView,
}: QCVerifiedDocumentItemProps) {
  const docTitle = doc.title || doc.file_name || `Dokumen_${doc.id}`;
  const uploaderName = getUploaderName(doc);
  const verifiedDate = getFormattedDate(doc.verified_at || doc.created_at);
      ...
      <div className="p-5 sm:p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="flex items-start gap-3.5 min-w-0">
            ...
            <div className="w-12 h-12 rounded-xl bg-[#DCFCE7] text-[#00C951] flex items-center justify-center shrink-0 shadow-2xs mt-0.5">
              <FileCheck className="w-6 h-6" />
            </div>

            <div className="min-w-0 space-y-1">
              <h3 className="text-base font-bold text-foreground truncate hover:text-emerald-600 transition-colors">
                {docTitle}
              </h3>
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1">
                  ...
                  <span>Diverifikasi pada {verifiedDate}</span>
                </span>
                <span>•</span>
                <span className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Badge
              variant="secondary"
              className="bg-[#DCFCE7] text-[#008732] border-0 font-semibold text-xs py-0.5 px-3 rounded-full shadow-2xs flex items-center gap-1"
            >
