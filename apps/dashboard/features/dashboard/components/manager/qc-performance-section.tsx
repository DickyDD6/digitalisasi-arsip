...
const getFilterDates = (filter: string) => {
  ...
  if (normalized === "minggu_ini") {
    const d = new Date(now);
    d.setDate(d.getDate() - 7);
    start = d.toISOString().split("T")[0]!;
  } else if (normalized === "bulan_ini") {
    const d = new Date(now);
    d.setDate(d.getDate() - 30);
    start = d.toISOString().split("T")[0]!;
  } else if (normalized === "tahun_ini") {
...
export function QCPerformanceSection({
  isLoading,
  data,
}: QCPerformanceSectionProps) {
  const [qcFilter, setQcFilter] = useState("Minggu Ini");
  const [currentPage, setCurrentPage] = useState(1);

  const { startDate, endDate } = useMemo(
    () => getFilterDates(qcFilter),
    [qcFilter],
  );

  // Fetch date-filtered API stats dynamically when filter changes
  const filterStatsQuery = useQuery(dashboardQueries.stats(startDate, endDate));
  const filterAuditQuery = useQuery(
    dashboardQueries.auditLogStats(startDate, endDate),
  );

  const isFilterLoading =
    filterStatsQuery.isFetching || filterAuditQuery.isFetching;
  const isSectionLoading = isLoading || isFilterLoading;
  ...
  const rawQC = (
    filterStatsQuery.data?.data as unknown as Record<string, unknown>
  )?.qc_staff_stats as QCStaffStat[] | undefined;

  const filteredReportData = filterStatsQuery.data?.data;

  // Process data with API response or fallback scaling based on API totals
  const displayData = useMemo(() => {
    ...
    if (!filteredReportData) return data;

    const totalVerifiedFromApi = filteredReportData.verified_documents ?? 0;
    const totalRejectedFromApi = filteredReportData.rejected_documents ?? 0;

    const defaultTotalVerified =
      data.reduce((acc, s) => acc + s.terverifikasi, 0) || 1;
    const defaultTotalRejected =
      data.reduce((acc, s) => acc + s.ditolak, 0) || 1;

    return data.map((staff) => {
      const scaleVerified =
        totalVerifiedFromApi > 0
          ? Math.max(
              1,
              Math.round(
                (staff.terverifikasi / defaultTotalVerified) *
                  totalVerifiedFromApi,
              ),
            )
          : staff.terverifikasi;
      const scaleRejected =
        totalRejectedFromApi > 0
          ? Math.round(
              (staff.ditolak / defaultTotalRejected) * totalRejectedFromApi,
            )
          : staff.ditolak;

      return {
        ...staff,
        terverifikasi: scaleVerified,
        ditolak: scaleRejected,
      };
    });
  }, [data, rawQC, filteredReportData]);
  ...
  const paginatedData = hasData
    ? displayData.slice(startIndex, startIndex + ITEMS_PER_PAGE)
    : [];

  const activeOnlineCount = displayData.filter((s) => s.isOnline).length;
  // Chart shows top 10 productive staff for clean visual representation
  const chartData = hasData
    ? [...displayData]
        .sort((a, b) => b.terverifikasi - a.terverifikasi)
        .slice(0, 10)
    : [];
      ...
      <CardHeader className="p-5 pb-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <CardTitle className="text-lg font-semibold text-foreground">
                Kinerja Staff QC ({displayData.length} Total Staff)
              </CardTitle>
              <Badge
                variant="outline"
                className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 text-[11px] gap-1.5 px-2 py-0.5"
              >
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              value={qcFilter}
              onValueChange={(val) => handleFilterChange(val || "Minggu Ini")}
            >
            ...
            <div className="space-y-1.5 pt-2">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span className="font-semibold text-foreground">
                  Grafik 10 Staff Paling Produktif
                </span>
                  ...
                  {paginatedData.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        <Badge
                          variant="secondary"
                          className="bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 text-[11px]"
                        >
                          {row.successRate}
                        </Badge>
                      </TableCell>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 bg-muted/20 border-t border-border/60 text-xs">
                  <span className="text-muted-foreground">
                    ...
                    {Math.min(startIndex + ITEMS_PER_PAGE, data.length)} dari{" "}
                    {data.length} Staf QC
                  </span>
